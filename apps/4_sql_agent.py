...
### db , llm , tools , agent , system_prompt 
...
db.run("""
    CREATE TABLE IF NOT EXISTS tasks(
        id INTEGER PRIMARY KEY AUTOINCREMENT,sql_
        title TEXT NOT NULL,
        description TEXT,
...
3. If the user requests a list of tasks, present the output in a structured table format to ensure a clean and organized display. 
...
@st.cache_resource    ## To be able to load the same chat 
def get_agent():
    agent = create_agent(
    model=model,
    tools=tools,
...
st.subheader("SQLBot-Manage your Tasks")
...
# for tool in tools:
#     print(tool.name)   ## sql_db_query , sql_db_schema , sql_db_list_tables , sql_db_query_checker
